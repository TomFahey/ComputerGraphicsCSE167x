...
void display() 
{
  ...
  if (numused) {
    ...
    vec4 lightPosition;
    vec4 lightTransform;
    GLfloat transfArray[4];
    GLfloat colorArray[4];
    int j;
    for (int i = 0; i < numused; i++)
    {
        j = 4 * i;
        lightPosition = vec4(lightposn[j], lightposn[j+1], lightposn[j+2], lightposn[j+3]);
        lightTransform = modelview * lightPosition;
        lightransf[j] = lightTransform[0];
        lightransf[j+1] = lightTransform[1];
        lightransf[j+2] = lightTransform[2];
        lightransf[j+3] = lightTransform[3];
    }
  ...
  for (int i = 0 ; i < numobjects ; i++) {
    ...
    rightmultiply(obj->transform, transformstack);
    modelview = transformstack.top();
